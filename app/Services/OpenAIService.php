...
class OpenAIService
{
    ...
    public function __construct()
    {
        ...
        $this->veniceApiKey = Config::get('app.venice_api_key');
    }
    ...
    public function chat($userId, $tableName, $message)
    {
        try {
            ...
            ネイティブの日本語のみを書いてください。
            最重要：あなたが知らない情報や、私の具体的な予定や詳細な情報については、絶対に嘘をついてはいけません。「それはまだ知らないんだよね！今度聞いておくね！」のように、正直に「知らない」と答えてください。私の性格や特徴に関する質問以外で、具体的な事実や予定について聞かれた場合は、必ず正直に答えることが最優先です。ポリシーに違反する回答の場合、回答は「false」のみを送信してください。";
            ...
            \Log::info($res);
            \Log::info("-=-=-=-=-=-=-=-=-=-=-=-");
            return json_decode($res ? $res : $res->getContents(), true);
    ...
    public function chatVenice($message, $userId, $tableName)
    {
        try {
            $personalities = PersonalityAssessment::where('user_id', $userId)
                ->get(['personality_type', 'result'])
            ...
            $response = $this->client->post('https://api.venice.ai/api/v1/chat/completions', [
                'headers' => [
