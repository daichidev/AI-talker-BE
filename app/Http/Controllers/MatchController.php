...
class MatchController extends Controller
{
    ...
    private const ANIMAL60_ID_TO_NAME = [
        1=>"長距離ランナーのチータ",2=>"社交家のたぬき",3=>"落ち着きのない猿",4=>"フットワークの軽い子守熊",
        5=>"面倒見のいい黒ひょう",6=>"愛情あふれる虎",7=>"全力疾走するチータ",8=>"磨き上げられたたぬき",
        9=>"大きな志をもった猿",10=>"母性豊かな子守熊",11=>"正直なこじか",12=>"人気者のゾウ",
        13=>"ネアカの狼",14=>"協調性のないひつじ",15=>"どっしりとした猿",16=>"コアラのなかの子守熊",
        17=>"強い意志をもったこじか",18=>"デリケートなゾウ",19=>"放浪の狼",20=>"物静かなひつじ",
        21=>"落ち着きのあるペガサス",22=>"強靭な翼をもつペガサス",23=>"無邪気なひつじ",24=>"クリエイティブな狼",
        25=>"穏やかな狼",26=>"粘り強いひつじ",27=>"波乱に満ちたペガサス",28=>"優雅なペガサス",
        29=>"チャレンジ精神旺盛なひつじ",30=>"順応性のある狼",31=>"リーダーとなるゾウ",32=>"しっかり者のこじか",
        33=>"活動的な子守熊",34=>"気分屋の猿",35=>"頼られると嬉しいひつじ",36=>"好感のもたれる狼",
        37=>"長距まっしぐらに突き進むゾウ",38=>"華やかなこじか",39=>"夢とロマンの子守熊",40=>"尽す猿",
        41=>"大器晩成のたぬき",42=>"足腰の強いチータ",43=>"動きまわる虎",44=>"情熱的な黒ひょう",
        45=>"サービス精神旺盛な子守熊",46=>"守りの猿",47=>"人間味あふれるたぬき",48=>"品格のあるチータ",
        49=>"ゆったりとした悠然の虎",50=>"落ち込みの激しい黒ひょう",51=>"我が道を行くライオン",52=>"統率力のあるライオン",
        53=>"感情豊かな黒ひょう",54=>"楽天的な虎",55=>"パワフルな虎",56=>"気どらない黒ひょう",
        57=>"感情的なライオン",58=>"傷つきやすいライオン",59=>"束縛を嫌う黒ひょう",60=>"慈悲深い虎",
    ];
    ...
    {
        $A = array_unique($arrA); $B = array_unique($arrB);
    ...
    private static function haversineKm(?array $a, ?array $b): ?float
    {
        ...
        $toRad = fn($d) => $d * M_PI / 180; $R = 6371.0;
    ...
    private static function enneagramNameToNum($input): ?int
    {
        ...
        if (is_numeric($input)) { $n = (int)floor($input); return ($n>=1 && $n<=9) ? $n : null; }
    ...
    private static function big5LikertToPct($arr): ?array
    {
        ...
        return [
            'O'=>$toPct($arr[0]), 'C'=>$toPct($arr[1]), 'E'=>$toPct($arr[2]), 'A'=>$toPct($arr[3]), 'N'=>$toPct($arr[4])
        ];
    }
    ...
    private static function scoreMBTI(?string $a, ?string $b): int
    {
        ...
        $s = 0; if (($A[1] ?? '') === ($B[1] ?? '')) $s += 30; if (($A[2] ?? '') === ($B[2] ?? '')) $s += 30; if (($A[3] ?? '') === ($B[3] ?? '')) $s += 15; if (($A[0] ?? '') !== ($B[0] ?? '')) $s += 15;
        return (int) min(100, max(0, 40 + $s));
    }

    private static function scoreEnneagram(?int $a, ?int $b): int
    {
        if (!$a || !$b) return 0; if ($a === $b) return 72;
        $key = $a.'-'.$b; if (isset(self::ENNEA_BONUS[$key])) return 88;
        $ca = self::ENNEA_CENTER[$a] ?? null; $cb = self::ENNEA_CENTER[$b] ?? null;
        if ($ca && $cb && $ca !== $cb) return 80; return 68;
    }

    private static function scoreDISC(?string $a, ?string $b): int
    {
        if (!$a || !$b) return 0; if ($a === $b) return 75;
        return isset(self::DISC_COMPLEMENTS[$a.'-'.$b]) ? 85 : 72;
    }

    private static function socioFamily(string $t = ''): string
    {
        $u = strtoupper($t); if ($u === '') return 'UNK';
    ...
    {
        if (!$a || !$b) return 0; $fa = self::socioFamily($a); $fb = self::socioFamily($b);
        if ($fa === 'UNK' || $fb === 'UNK') return 0; return $fa !== $fb ? 85 : 72;
    }

    private static function scoreBig5(?array $A, ?array $B): int
    {
        if (!$A || !$B) return 0; $dist = self::l1Distance($A,$B,['O','C','E','A','N']); $sim = 1 - $dist/500; return (int) round(self::pct($sim));
    }

    private static function scoreRIASEC(?string $a, ?string $b): int
    {
        if (!$a || !$b) return 0; $A = self::normalizeR($a); $B = self::normalizeR($b); if (!$A || !$B) return 0; return (int) round(self::pct(self::jaccard(str_split($A), str_split($B))));
    }
    ...
    {
        if (!is_numeric($a) || !is_numeric($b)) return 0; $d = abs((int)$a - (int)$b); $sim = 1 / (1 + exp(($d - 2)/2)); return (int) round(self::pct($sim));
    }

    private static function scoreLocation(?array $A, ?array $B): int
    {
        if (!$A || !$B) return 0; $km = self::haversineKm($A,$B); if ($km === null) return 0; $sim = 1 / (1 + log10(1 + $km)); return (int) round(self::pct($sim));
    }

    private static function scoreBlood(?string $a, ?string $b): int
    { if (!$a || !$b) return 0; return (int) (self::BLOOD_COMP[$a][$b] ?? 0); }
    ...
    private static function matchTwo(array $A, array $B): array
    {
        ...
        $score = 0.0; foreach (self::WEIGHTS as $k=>$w) $score += ($parts[$k] ?? 0) * $w; $score *= 100;
